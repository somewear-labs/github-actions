...
// Internal-vs-external check using the PR webhook payload's author_association
// field. Possible values per GitHub docs:
//   OWNER, MEMBER, COLLABORATOR  — trust as internal contributor
//   CONTRIBUTOR, FIRST_TIMER, FIRST_TIME_CONTRIBUTOR, MANNEQUIN, NONE — external
//
// We use the payload field instead of calling github.rest.orgs.checkMembershipForUser
// because GITHUB_TOKEN under Actions cannot read org membership without elevated
// permissions that consumers don't grant. The payload value is set by GitHub
// at webhook-emission time using the same authoritative data.
const INTERNAL_ASSOCIATIONS = new Set(['OWNER', 'MEMBER', 'COLLABORATOR']);

function isInternalContributor(pr) {
...
module.exports = async function main({ github, context, core }) {
  ...
  //
  // Uses pr.author_association from the webhook payload (no API call), so
  // we don't need org:read permissions that consumers' GITHUB_TOKEN can't
  // grant. OWNER/MEMBER/COLLABORATOR are treated as internal; CONTRIBUTOR /
  // FIRST_TIMER / FIRST_TIME_CONTRIBUTOR / MANNEQUIN / NONE are external.
  if (!isInternalContributor(pr)) {
    await ensureComment({
      github, context,
      kind: 'external',
      body: `Skipping ticket creation for external contributor \`@${pr.user.login}\` (author_association=${pr.author_association}).`
    });
    decision(core, 'skip-external', `${pr.user.login} association=${pr.author_association}`);
    return;
  }
